...
from setting import Circle, Regular4, PriorProcesser
...
parser.add_argument('--mode', choices=['low_test_error_preference', 'similar_input_distribution_preference'], default='low_test_error_preference')
...
if 1: # define parameters
    ...
    context_len_list = [f'$2^{int(np.log2(l))}$' for l in k_list]
...
if 1: # set 4x5
    resize = 8
    fig, axs = plt.subplots(3, 5, figsize=(8.3*resize, 3*resize), sharex=False, sharey='row')



if 1: # draw 1x5 for the test risk of each pretraining function classes
    row_index = 0
    for row, (sm, sw) in enumerate(zip(sm_list, sw_list)):
        ...
        data = eval_loss_of_pt_tasks = np.load(folder+'eval_loss_of_pt_tasks.npy')
        print(row, data)
        x = np.arange(len(data))
        labels = [f'Test err of PT task {i+1}' for i in range(M)]
        
        for i in range(len(data)):
            ax.bar(x[i], data[i], color=colors[i], label=labels[i])
        plt.legend()
        ax.set_xticks(np.arange(len(data)))
        ax.set_xticklabels([f'T{i+1}' for i in x])
        
        ax.set_title(r'$\delta_{\mu}=\delta_{w}=$'+' '+sw_strs[row])
        if args.mode == 'low_test_error_preference':
            test_err_max = 600
        elif args.mode == 'similar_input_distribution_preference':
            test_err_max = 12000
        test_err_min = 0
        ax.set_ylim([test_err_min,test_err_max])

        if row == len(sm_strs)-1:
            ax.legend(loc='right', bbox_to_anchor=(1.13*legend_dis, 0.5),fontsize = S*0.85)     
...
if 1: # draw 1x5 for pi
    ...
    for row, (sm, sw) in enumerate(zip(sm_list, sw_list)):
        ...
        ax.set_xticklabels(context_len_list)
        for m in range(M):
            TV = B_tpis[:,k_array,m] 
            ...
            ax.plot(means, label=r'PT task weight $\tilde{{\pi}}_{{{}}}$'.format(m+1), linewidth=18, color=colors[m])
        ...
        if row == len(sm_strs)-1:
            ax.legend(loc='right', bbox_to_anchor=(1.1*legend_dis, 0.5), fontsize = S*0.9)


if 0: # draw 1x5 for distance of task retrieval 1
    row_index = 0
    for row, (sm, sw) in enumerate(zip(sm_list, sw_list)):
        folder = 'data/'+fig_name+'/sm='+str(sm)+' sw='+str(sw)+'/'
        print('\n***** ' +str(row)+ '-' +str(0)+ ' *****')
        ...
        for m in range(M):
            ...
            ax.plot(means, label=r'$\|\tilde{{{{w}}}}_{{{}}} - {{w}}^*\|$'.format(m+1), linewidth=linewidth, color=colors[m])
            ax.fill_between(range(len(means)), lower_bound, upper_bound, color=colors[m], alpha=0.15)
        ...
        if row == len(sm_strs)-1:
            ax.legend(loc='right', bbox_to_anchor=(legend_dis, 0.5), fontsize = S*1.0)

    
if 0: # draw 1x5 for distance of task learning 2
    row_index = 2
    for row, (sm, sw) in enumerate(zip(sm_list, sw_list)):
        folder = 'data/'+fig_name+'/sm='+str(sm)+' sw='+str(sw)+'/'
        print('\n***** ' +str(row)+ '-' +str(0)+ ' *****')
        ...
        ax.plot(np.arange(len(k_list)), means, 'o-', label=r'$\|\tilde{{{{w}}}} - {{w}}^*\|$', linewidth=linewidth, color='black')#'blue')
...
if 4: # draw 1x5 for prediction
    ...
    for row, (sm, sw) in enumerate(zip(sm_list, sw_list)):
        ...
        upper_bound = means + margin_error
        print(means)
        ax.plot(np.arange(len(k_list)), means, 'o-', label=r'$(\mathcal{F}^* - y_{T+1}^{*})^2$', linewidth=linewidth, color='black')#'blue')
        ax.fill_between(range(len(means)), lower_bound, upper_bound, color='black', alpha=0.15)
        ...
        if row == len(sm_strs)-1:
            ax.legend(loc='right', bbox_to_anchor=(legend_dis, 0.5), fontsize = S*0.9)
...
if 1:
    ...
    plt.tight_layout(pad=0.25, h_pad=0.07, w_pad=0.25)
    plt.savefig(f'{args.mode}.pdf',bbox_inches='tight')
    plt.savefig(f'{args.mode}.jpg',bbox_inches='tight')
