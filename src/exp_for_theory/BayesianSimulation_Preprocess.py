...
data_dim = 3
hidden_dim = 100 
W1 = torch.randn(data_dim, hidden_dim)
W2 = torch.randn(hidden_dim, 1)
...
if 1: # preprocess
    for index, (sm, sw) in enumerate(zip(sm_list,sw_list)):
        ...
        print('\n***** ' +str(index)+ ' *****')
        prior = Regular4(sm=sm,sw=sw, mode=args.mode)
        ...
        if 1:
            ...
            colors = ['#CD1818', '#0E21A0', '#116D6E', '#331D2C']
            if 1:
                bs_xs, bs_ys, bs_retrieval, bs_learning, eval_loss_of_pt_tasks = priorProcesser.draw_demon_sequences(K, num_k, W1, W2)
                ...
                np.save(folder+'eval_loss_of_pt_tasks.npy', eval_loss_of_pt_tasks)
                print(eval_loss_of_pt_tasks)
            else:
